...
- `OC_DB` (default `~/.local/share/opencode/opencode.db`), `OC_DEV_ROOT` (default `~/dev`, where the git repos live), `OC_GIT_AUTHORS` (optional comma-separated author emails or names added to the learned set; identities are otherwise learned from file-overlap plus `git config user.email`), `OC_INSIGHTS_CACHE` (default `~/.local/share/ocInsights/data.json`).
...
- **Window** — all time, or the last 90 / 45 / 30 / 7 days, anchored to the latest day with activity (not the wall clock, so the deck is stable). On the turns and overall cards a window changes which cycles are scored, not the yardstick: the pool stays the all-time pool for the same grouping and session filter, so a week reads against your usual level. A window ranks every group with a judged cycle (all time needs 10); the evidence weighting pulls a group with one or two cycles close to 50, so it only stands out when its cycles do. When fewer than two groups rank, the ranking says so and suggests a wider window, family grouping or Role Off.
