...
**Build sessions.** `build`, `Sisyphus-Junior`, `Sisyphus (Ultraworker)`, `BuildAgent`, `beads-task-agent`, `general`. The productivity and shipping maps default to these because explore, plan and librarian sessions read by design and would make any model used for exploring look unproductive. **Plan sessions** are `plan` and `Metis (Plan Consultant)`.
