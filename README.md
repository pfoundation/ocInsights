...
verify.mjs   opens the built deck in headless Chromium, 87 checks
...
**Overall score.** Ten axes in five tiers, equal weights: delivery per turn (turns to ship, ship rate, edits per turn) · throughput (edits per hour, verified sessions, median hours to ship) · quality (edit reliability, tool errors) · autonomy (replies per turn) · cost (dollars per shipped tree). The score is the mean of the ten pool-relative positions, 0 to 100 with 50 at pool, missing axes dropped. Turns, edits, errors and cost come from session trees; throughput and quality from session sums. Commit-window metrics (hours per commit, lines per commit) are excluded — keyed per commit, not per session group. Same strips/radar mechanics as the turns card, ranked by score. Everywhere in the deck a prompt is one of your turns on a top-level session; subagent delegation prompts are excluded. Two views share the six axes: the radar is the default, with no axis labels — hovering a vertex names its spoke; strips draw raw-value rows — better to the right, or ascending with the better end arrowed — with connector lines joining each of the top 8 groups' dots. In the radar, raw draws outward-is-more on every axis (a rim turns vertex is worse); core stays better = 100 either way.
