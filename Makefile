.PHONY: all extract build verify publish-npm clean template install-plugin ledger smoke smoke-http serve typecheck contribute contribute-preview
...
smoke:                            ## prove the ledger hook records: one write in a scratch dir through a fresh opencode process
	@d=/tmp/opencode/ledger-smoke; f=$${OC_EDIT_LEDGER:-$$HOME/.local/share/ocInsights/edits.jsonl}; n=$$(grep -c "$$d/hello.txt" "$$f" 2>/dev/null || echo 0); \
